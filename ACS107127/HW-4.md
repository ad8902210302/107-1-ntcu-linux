...
####  用"mkdir"建立新目錄，用"chmod"更改權限，再用"ll"檢視。</br>
...
####  用"chmod u+s"給予SUID的權限(只要任何人有x的執行權，當用戶執行xxx時，會自動透過SUID轉換身分為owmer，亦變為root的身分)。</br>
####  執行【myls /srv/myproject】
...
####  用"ps aux |grep rsyslog"觀察程序，並用">"將資料移轉到指定檔案，並用"cat"檢視檔案內容。</br>
